...
flowchart TD
    I["IDENTIDADE ÚNICA"]
    I --> C["Consumidor"]
    I --> L["Lojista / Empreendedor"]
    I --> F["Funcionário"]
    I --> E["Empregador"]
    I --> V["Candidato"]
    I --> S["Prestador de serviços"]
    I --> P["Fornecedor"]
    I --> B["Comprador / Vendedor B2B"]
    I --> R["Valley Rider"]
    I --> O["Operações e logística"]
    I --> PAR["Parceiro"]
```
...
```mermaid
flowchart LR
    A["Empresa entra no AIO"] --> B["Perfil e contexto"]
    B --> C["Necessidades"]
    C --> D["Capacidades relevantes"]
    D --> E["Uso"]
    E --> F["Empresa evolui"]
    F --> G["Novas necessidades"]
    G --> D
```
...
```mermaid
pie showData
    title Participação indicada pelo MEMP no total de CNPJs registrados
    "Micro e pequenas empresas — 96%" : 96
    "Demais — 4%" : 4
```
...
```mermaid
flowchart TD
    U["Usuário"] --> R["Área de atendimento"]
    R --> A["Loja próxima"]
    R --> B["Restaurante próximo"]
    R --> C["Prestador próximo"]
    R --> D["Serviço local"]
    X["Fora da área"] -. "não precisa disputar a mesma atenção" .-> R
```
...
```mermaid
flowchart LR
    A["AIO — Operação"] --> B["Empresa"]
    B --> C["Valley — Distribuição"]
    C --> D["Consumidor"]
    D --> E["Pedido / Serviço"]
    E --> F["Valley Riders — Logística"]
    F --> D
    D --> G["Relacionamento"]
    G --> B
```
...
```mermaid
flowchart LR
    G["GOLDEN — Lojista"] --> E["Ecossistema"]
    E --> P["PEPITA — Usuário"]
    P --> C["Recorrência"]
    C --> E
    E --> G
...
```mermaid
flowchart LR
    M["25,2 milhões de empresas ativas"]
    M --> A["0,01% = 2.520 empresas"]
    M --> B["0,10% = 25.200 empresas"]
    M --> C["1,00% = 252.000 empresas"]
```
...
```mermaid
flowchart TD
    N["Novas empresas entram no mercado"]
    N --> A["AIO resolve uma necessidade"]
    A --> P["Plataforma entende melhor o perfil"]
    P --> X["Novas capacidades tornam-se relevantes"]
    X --> V["Empresa ganha eficiência"]
    V --> Y["Valley amplia distribuição"]
    Y --> U["Mais usuários encontram a empresa"]
    U --> T["Transações, serviços e relacionamento"]
    T --> G["Golden / Pepita"]
    G --> R["Maior recorrência"]
    R --> X
```
